...
# Read keys from "map_wnid_to_classes.txt" and store them in a set
with open("map_wnid_to_classes.txt", "r") as file:
    allowed_keys = set(line.split()[0] for line in file)

# Filter class_info_dict to include only allowed keys 
# this selects only the 1000 classes present within the dataset Imagenet1K
class_info_dict = {key: val for key, val in class_info_dict.items() if key in allowed_keys}
...
def get_image(class_name, img_url):
    ...
    # img_file_path = os.path.join(imagenet_images_folder, class_name, img_name)
...
if __name__ == '__main__':
    ...
    for class_wnid in classes_to_scrape:
        ...
        part = partial(get_image, class_name)
