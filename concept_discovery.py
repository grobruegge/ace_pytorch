...
class ConceptDiscovery(object):
  ...
  def load_images_from_folder(self, folder_name: str, max_imgs:int=1000):
    ...
    return helpers.load_images_from_files(
  ...
  def _load_or_calc_acts(self, model, imgs:np.ndarray, identifier:str, bs:int=100) -> dict:
    ...
    layer_activations = helpers.get_layer_activations(model, self.layer, imgs, bs)
  ...
  def save_concepts(self):
    ...
    for concept in tqdm(self.concept_dict['concepts'], desc='[INFO] saving concept images/patches'):
      ...
      helpers.save_images(patch_addresses, patches)
      helpers.save_images(image_addresses, images)
